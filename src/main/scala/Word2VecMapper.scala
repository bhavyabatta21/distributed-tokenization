...
class Word2VecMapper extends Mapper[LongWritable, Text, Text, Text] { // Changed Text to LongWritable
  ...
  override def map(key: LongWritable, value: Text, context: Mapper[LongWritable, Text, Text, Text]#Context): Unit = {
    ...
    // Load tokenized data
    val tokenizedData: List[List[Long]] = readTokenizedFile("s3a://distributed-llm-bb-part-1/output/part-r-00000", context)
  ...
  def readTokenizedFile(filePath: String, context: Mapper[LongWritable, Text, Text, Text]#Context): List[List[Long]] = {
    ...
    conf.set("fs.defaultFS", "s3a://distributed-llm-bb-part-1") // Use s3a for better performance
