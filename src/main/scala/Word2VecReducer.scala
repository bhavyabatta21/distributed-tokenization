...
class Word2VecReducer extends Reducer[Text, Text, Text, Text] {
  ...
  def setupModel(context: Context): Unit = {
    val modelS3Path = new Path("s3a://distributed-llm-bb-part-1/model/word2Vec-model.bin")  // S3 model path
    val conf = context.getConfiguration
    conf.set("fs.defaultFS", "s3a://distributed-llm-bb-part-1")
    val fs = FileSystem.get(new URI("s3a://distributed-llm-bb-part-1"), conf)

    // Define a local path where you temporarily store the model
    val localModelPath = new java.io.File("s3a://distributed-llm-bb-part-1/model/word2vec_model.bin")
